...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE news SET titulo=%s, resumo=%s, foto=%s WHERE id=%s",
...
$colname_papel = "-1";
if (isset($_GET['id'])) {
  $colname_papel = (get_magic_quotes_gpc()) ? $_GET['id'] : addslashes($_GET['id']);
}
mysql_select_db($database_conexao, $conexao);
$query_papel = sprintf("SELECT * FROM news WHERE id = %s", $colname_papel);
$papel = mysql_query($query_papel, $conexao) or die(mysql_error());
$row_papel = mysql_fetch_assoc($papel);
$totalRows_papel = mysql_num_rows($papel);
?>
<?php require_once('../Connections/conexao.php'); ?>
		...
		<form action="<?php echo $editFormAction; ?>" method="POST" name="form" id="form" accept-charset="iso-8859-1">
			...
			<?php }?>
				  <table width="400" border="0" cellpadding="2" cellspacing="3" class="texto_formulario">
                    ...
                    <tr>
                      <td colspan="3"><input name="titulo" type="text" class="form2" id="titulo" value="<?php echo $row_papel['titulo']; ?>" size="70" /></td>
                    </tr>
                    <tr>
                      <td colspan="3">&nbsp;</td>
                    </tr>
                    ...
                    <tr>
                      <td colspan="3"><input name="foto" type="text" class="form2" id="foto" value="<?php echo $row_papel['foto']; ?>" size="70" /></td>
                    </tr>
                    <tr>
                      <td colspan="2">&nbsp;</td>
                      <td >&nbsp;</td>
                    ...
                    <tr>
                      <td colspan="3"><input name="resumo" type="text" class="form2" id="resumo" value="<?php echo $row_papel['resumo']; ?>" size="70" /></td>
                    </tr>
                    <tr>
                      <td colspan="2">&nbsp;</td>
                      <td >&nbsp;</td>
                    ...
                    <tr>
                      <td colspan="2"><input name="vid" type="hidden" id="vid" value="<?php echo $row_papel['id']; ?>" /></td>
                      <td >&nbsp;</td>
                    </tr>
